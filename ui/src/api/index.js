...
function pathId(id) {
  return encodeURIComponent(toIdString(id));
}
...
export async function fetchProject(id) {
  const result = await request(`/projects/${pathId(id)}`);
  return unwrapResult(result);
}

export async function fetchEcus(projectId, { page = 1, size = 12 } = {}) {
  const result = await request(`/projects/${pathId(projectId)}/ecus?page=${page}&size=${size}`);
  return unwrapResult(result);
}

export async function fetchEcu(projectId, ecuId) {
  const result = await request(`/projects/${pathId(projectId)}/ecus/${pathId(ecuId)}`);
  return unwrapResult(result);
}
...
export async function createEcu(projectId, payload) {
  const result = await request(`/projects/${pathId(projectId)}/ecus`, {
    method: 'POST',
...
export async function batchDeleteProjects(ids) {
  const result = await request('/projects/batch-delete', {
    method: 'POST',
    body: JSON.stringify({ ids: (ids || []).map(toIdString) })
  });
  return unwrapResult(result);
}

export async function batchDeleteEcus(projectId, ids) {
  const result = await request(`/projects/${pathId(projectId)}/ecus/batch-delete`, {
    method: 'POST',
    body: JSON.stringify({ ids: (ids || []).map(toIdString) })
  });
  return unwrapResult(result);
